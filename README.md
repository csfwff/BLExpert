...
- 脚本源码、输入/输出帧和日志已有大小上限；Android/Windows/Linux 的 QuickJS 已启用 50ms/16MiB 硬限制，脚本写入按 200ms 限流，改写帧与高风险关键词命令会要求确认。工作区命令白名单和设备发送策略已接入真实发送路径；Apple JavaScriptCore 的可终止超时及完整目标平台审计仍未完成。
